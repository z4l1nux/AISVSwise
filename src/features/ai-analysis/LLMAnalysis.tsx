...
export default function LLMAnalysis({
    ...
    const isSeparatorRow = (line: string) => isTableRow(line) && /^\|[\s|:-]+\|$/.test(line.trim());
    const parseTableRow = (line: string): string[] =>
        line.trim().slice(1, -1).split('|').map(c => c.trim());

    const flushTable = (rows: string[][], key: string): React.ReactNode => {
        ...
        const tdStyle: React.CSSProperties = {
            ...
            fontSize: '13px', color: '#cbd5e0', verticalAlign: 'top',
        };
        const thStyle: React.CSSProperties = {
            ...tdStyle, color: '#94a3b8', fontWeight: 700,
            borderBottom: '2px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.04)',
        };

        return (
            <div key={key} style={{ overflowX: 'auto', margin: '10px 0 14px' }}>
    ...
    const renderMarkdown = (text: string): React.ReactNode[] => {
        ...
        const flushTableBuffer = (flushIdx: number | string) => {
            if (tableBuffer.length) {
                const node = flushTable(tableBuffer, `table-${flushIdx}`);
        ...
        lines.forEach((line, idx) => {
            if (isTableRow(line)) {
                flushList(idx);
                tableBuffer.push(isSeparatorRow(line) ? parseTableRow(line) : parseTableRow(line));
            } else if (line.startsWith('## ')) {
                flushList(idx); flushTableBuffer(idx);
