...
|-------------|:---:|-------|
| 9.5.1 Mutual auth and encryption (TLS 1.3+) | Mature | TLS 1.3 is well-established. Service meshes (Istio, Linkerd) and Dapr's Sentry service automate mTLS with SPIFFE ID issuance and certificate rotation. A2A supports declared auth schemes (OAuth 2.0, OIDC, mTLS) via Agent Cards. The MCP spec evolved through three security revisions in 2025: March (OAuth 2.1 baseline), June (Protected Resource Metadata per RFC 9728), and November (mandatory PKCE, Client ID Metadata Documents). Gap: despite spec maturity, 43% of deployed MCP servers still have OAuth authentication flaws as of early 2026 (CoSAI survey). |
