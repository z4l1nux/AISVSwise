...
> **2025-2026 Highlights:** The MCP spec matured significantly with OAuth 2.1 authorization and streamable HTTP transport replacing legacy SSE. Tool poisoning and rug-pull attacks were publicly disclosed, driving new requirements for component allowlisting (C10.1) and tool definition change detection (C10.4). As of March 2026, 50+ CVEs have been filed against MCP components (including CVE-2026-23744 at CVSS 9.8 — the first critical-severity RCE in MCP tooling), a scan of 5,618 servers found only 2.5% verified safe, and the first malicious MCP server (postmark-mcp) demonstrated real-world supply chain compromise. The CoSAI MCP Security Taxonomy (January 2026) formalized ~40 threats across 12 categories. Enterprise adoption is accelerating with Microsoft's C# SDK v1.0, Proofpoint Secure Agent Gateway, and Salesforce Agentforce MCP support all shipping in Q1 2026.
...
- **MCP security gateways:** [Lasso MCP Gateway](https://github.com/lasso-security/mcp-gateway/) (open-source, April 2025 — prompt injection blocking, data leakage prevention via Presidio, server reputation vetting, tool filtering), [Enkrypt AI Secure MCP Gateway](https://www.enkryptai.com/mcp-scan) (commercial, automated static analysis), [Proofpoint Secure Agent Gateway](https://www.proofpoint.com/us/blog/corporate-news/stop-ai-agents-losing-data) (Q1 2026 — MCP-native DLP for agentic workflows, policy enforcement on data flowing into/out of agents, works with all MCP-compliant agents), [Salesforce Agentforce MCP Gateway](https://www.salesforce.com/agentforce/mcp-support/) (beta — trusted gateway with allowlisting, tool poisoning defense, transforms MCP servers into governed actions within the Agentforce Asset Library)
...
|--------------|:---:|-------|
| C10.1 Component Integrity & Supply Chain | Emerging | No standardized signing for MCP packages. The postmark-mcp incident (first malicious MCP server in the wild) validates this gap. Only 2.5% of 5,618 scanned servers verified safe. Snyk Agent Scan, Cisco MCP Scanner, and Invariant mcp-scan provide detection. Vulnerable MCP Project tracks 50 CVEs. Salesforce Agentforce MCP Gateway introduces allowlisting for enterprise deployments. CoSAI taxonomy (MCP-T6, MCP-T11) formalizes integrity and supply chain threat categories. |
...
| C10.5 Outbound Access & Agent Execution Safety | Emerging | Lasso MCP Gateway and Proofpoint Secure Agent Gateway provide proxy-level filtering and DLP. MCP sampling attacks (Unit 42) show egress and execution risks. CVE-2026-26118 (Azure MCP SSRF, CVSS 8.8) demonstrated managed identity token exfiltration via unconstrained outbound requests. CVE-2026-33010 (MCP Memory Service, CVSS 8.1) showed permissive CORS enabling cross-origin data theft. No standard framework for per-tool execution limits in MCP spec. |
| C10.6 Transport Restrictions & Boundary Controls | Emerging | Stdio isolation is ad-hoc. Host header validation middleware added to SDK. DNS rebinding protections now available but require explicit opt-in. CVE-2026-23744 (MCPJam Inspector RCE, CVSS 9.8) showed dangers of default 0.0.0.0 binding in development tools. CoSAI taxonomy (MCP-T7, MCP-T8) formalizes session/transport and network isolation threat categories. |
