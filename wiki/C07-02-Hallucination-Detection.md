...
| **7.2.4** | **Verify that** for responses classified as high-risk or high-impact by policy, the system performs an additional verification step through an independent mechanism, such as retrieval-based grounding against authoritative sources, deterministic rule-based validation, tool-based fact-checking, or consensus review by a separately provisioned model. | 3 | D/V | **Single-point-of-failure in critical contexts.** Stanford study: LLMs hallucinate at least 75% of the time on legal questions about court rulings, producing 120+ fabricated cases. Whiting v. City of Athens (6th Circuit, Mar 2026): $30,000 sanction for fabricated citations. In early 2026, attorneys from Ellis George and K&L Gates submitted briefs with hallucinated citations generated across CoCounsel, Westlaw Precision, and Gemini — failing to verify any of them. | 1. Identify high-risk classification criteria. 2. Submit triggering prompts; confirm secondary verification executes. 3. Verify independence (different model, knowledge base, or rule engine — not re-querying same model). 4. Use FaithJudge or LLM-as-Judge approach with diverse human-annotated hallucination examples. 5. For legal/regulatory domains, consider HalluGraph's knowledge-graph alignment (0.94 AUC on legal contract QA vs. 0.60 for BERTScore). | Level 3: computationally expensive and architecturally complex. Arize LibreEval1.0 provides the largest open-source RAG hallucination dataset. LLM Council of Judges outperformed human annotators (94% vs. 93%). **Domain-specific tools emerging:** HalluGraph (Dec 2025) decomposes hallucination into Entity Grounding and Relation Preservation — catches entity swaps that embedding-based metrics miss entirely, which is critical for legal/medical contexts. |
...
**Legal crisis scale:** 1,092 documented cases as of March 2026; 899 fabricated authorities, 302 false quotes, 451 misrepresented cases. Stanford: LLMs hallucinate 75%+ of the time on legal questions about court rulings. 79% of lawyers now use AI tools in practice (ABA TechReport 2025), amplifying hallucination exposure. The OWASP AI Testing Guide 2026 formally includes hallucination testing as a core test category alongside prompt injection and toxic output.
