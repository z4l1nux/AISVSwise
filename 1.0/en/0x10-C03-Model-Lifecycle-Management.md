...
|:--------:|---------------------------------------------------------------------------------------------------------------------|:---:|:---:|
| **3.1.1** | **Verify that** every released model, tokenizer, and pre-processing asset receives a incrementing semantic version (e.g., *major.minor.patch*). | 1 | D/V |
...
| # | Description | Level | Role |
|:--------:|---------------------------------------------------------------------------------------------------------------------|:---:|:---:|
| **3.2.1** | **Verify that** production endpoints support at least one rollback slot and that switch-overs complete in less than 5 minutes. | 1 | D |
...
| **3.2.3** | **Verify that** each patch passes the full security test-suite before traffic shifts beyond 5%. | 2 | V |
| **3.2.4** | **Verify that** rollback artifacts are signed and retained for one year or more to support forensic analysis. | 2 | V |
...
| **3.4.2** | **Verify that** logs are consolidated within 5 minutes into a centralized, write-once store. | 1 | D/V |
| **3.4.3** | **Verify that** log integrity is enforced via chain-hashing or Merkle proofs and validated nightly. | 2 | V |
| **3.4.4** | **Verify that** privileged log viewers require MFA and least-privilege access controls. | 2 | D |
| **3.4.5** | **Verify that** quarterly audits sample at least 10% of changes for policy compliance. | 3 | V |
...
| **3.5.4** | **Verify that** safety and policy evals (e.g., Toxicity, Jailbreak rate) block promotion when risk is greater than the agreed upon threshold. | 2 | D/V |
...
## C3.7 Model Version API Security

Secure management and access to different model versions through APIs and service endpoints.

| # | Description | Level | Role |
|:--------:|---------------------------------------------------------------------------------------------------------------------|:---:|:---:|
| **3.7.1** | **Verify that** API endpoints clearly identify which model version is being accessed and return version information in responses. | 1 | D/V |
| **3.7.2** | **Verify that** deprecated model versions return appropriate warnings and have clearly communicated sunset dates. | 2 | D/V |
| **3.7.3** | **Verify that** model version switching requires appropriate authorization and all version changes are logged with user context. | 2 | D/V |
| **3.7.4** | **Verify that** unauthorized attempts to access restricted model versions are blocked and trigger security alerts. | 2 | V |
| **3.7.5** | **Verify that** model version metadata is protected from unauthorized modification and tampering. | 3 | D/V |

---

## C3.8 Formal Decommissioning
...
| # | Description | Level | Role |
|:--------:|---------------------------------------------------------------------------------------------------------------------|:---:|:---:|
| **3.8.1** | **Verify that** a retirement request triggers a dependency scan to identify downstream services and queues.| 1 | D |
| **3.8.2** | **Verify that** model binaries, datasets, and feature logs are securely erased or archived per retention policy. | 1 | D/V |
| **3.8.3** | **Verify that** revoked model signatures are published to a public CRL (certificate-revocation list) to block re-use. | 2 | V |
| **3.8.4** | **Verify that** decommission events update inventory dashboards and notify owners of dependent systems. | 2 | D |
