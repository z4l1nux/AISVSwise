...
| **9.8.3** | **Verify that** a system-level cap on simultaneously active agent instances is enforced by the orchestration runtime, so that spawning loops or recursive sub-agent creation cannot cause unbounded swarm growth or resource exhaustion. | 1 | D/V |
| **9.8.4** | **Verify that** each agent can only access its own memory namespace, context window, and credentials, and is technically prevented from reading or modifying peer agent state, preventing lateral movement across agents within the same swarm. | 2 | D/V |
| **9.8.5** | **Verify that** swarm-level aggregate action rate limits (e.g., total external API calls, file writes, or network requests per time window across all agents) are enforced to prevent coordinated bursts that cause unintended impact on external systems. | 2 | D/V |
| **9.8.6** | **Verify that** a swarm-level emergency shutdown capability exists that halts all active agent instances simultaneously, revokes their tool and MCP access, and prevents new agent spawning, with the shutdown completable within a pre-defined response time. | 3 | D/V |
