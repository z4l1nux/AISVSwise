...
| # | Description | Level | Role |
|:--------:|---------------------------------------------------------------------------------------------------------------------|:---:|:---:|
| **11.10.1** | **Verify that** inference endpoints serving security-relevant classifiers (e.g., abuse detection, fraud scoring, malware classification) monitor query patterns for bias-pattern reconnaissance, such as systematic variation of a single demographic, linguistic, or stylistic dimension across otherwise similar inputs, and generate alerts when such patterns are detected. | 2 | D/V |
| **11.10.2** | **Verify that** adversarial robustness evaluations for security-relevant classifiers are stratified by meaningful input subgroups (e.g., language register, content category), and that per-subgroup false-negative rates under adversarial conditions are measured, documented, and flagged when they deviate from aggregate rates beyond a defined threshold. | 2 | D/V |
